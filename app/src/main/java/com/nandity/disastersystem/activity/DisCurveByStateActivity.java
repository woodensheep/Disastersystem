 ...
 * 灾险情诱发因素统计
 */
public class DisCurveByStateActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    private String TAG = "DisCurveByStateActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        context = DisCurveByStateActivity.this;
    ...
    private void initViews() {
        ...
        tvDistypeTitle.setText("任务灾害点统计");
    ...
    private void dataInit() {
        ...
        for (int i = 0; i < numColumns; ++i) {
            ...
            for (int j = 0; j < numSubcolumns; ++j) {
                switch (i){
                    case 0: values.add(new SubcolumnValue((float)mDisTypes.get(j).getJiu(), COLORS[j]));break;
                    case 1: values.add(new SubcolumnValue((float)mDisTypes.get(j).getXin(), COLORS[j]));break;
                }
    ...
    private void setOkHttp1() {
        progressDialog.show();
        try {
            OkHttpUtils.get().url(new ConnectUrl().getTaskDisCurveByStateUrl())
                            ...
                            try {
                                ...
                                if ("200".equals(status)) {
                                    ...
                                    for(int i=0;i<dmtx.length();i++){
                                        ...
                                        disType.setXin(message.getJSONArray("xin").getInt(i));
                                        disType.setJiu(message.getJSONArray("jiu").getInt(i));
                                    ...
                                    dataInit();
                                    setBiaoZhu();
                                 tvDisTypeName.setText(typeNames[0]);
